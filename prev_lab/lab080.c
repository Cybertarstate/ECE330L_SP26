...
if (pc11) {
	Seven_Segment_Digit(digit, sw);
}

GPIOD->ODR = (sw << 12) | (1 << digit);
...
int x = HexValue >> (4*digit) & 0xF;
Seven_Segment_Digit(digit, x  /*Replace 0 with correct HexValue
characters*/);
}
return;
}
